...
- **Subagents**: ultra (max compression, abbreviations, arrows) — cuts parent re-merge token cost; required for protocol envelope fields per `subagent-response-protocol`
...
Subagent traffic: follow `subagent-response-protocol` rule. Structured YAML envelope per `~/.config/opencode/templates/subagent-response.yml.tmpl`; hooks inject the contract and parse the response. See `~/.config/opencode/skills/subagent-response-protocol/`.
