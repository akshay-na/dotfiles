...
| Consumer                                                        | Role                                                                                                                                              |
| --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cursor/.opencode/agents/atlassian-pm.md`                          | Primary consumer. Calls this skill at session start when ≥ 2 issues will be created, or when the user supplies a parent / initiative key.         |
| `~/.config/opencode/plugins/cache/cursor-public/atlassian/.../skills/spec-to-backlog/SKILL.md`                | After the G2.5 retrofit, `atlassian-pm` invokes this skill before drafting backlog items derived from a Confluence spec page.                     |
| `~/.config/opencode/plugins/cache/cursor-public/atlassian/.../skills/capture-tasks-from-meeting-notes/SKILL.md` | After the G2.5 retrofit, `atlassian-pm` invokes this skill before drafting tasks captured from a Confluence meeting-notes page.                   |
| `cursor/.opencode/agents/atlassian-pm.md` — plugin-skill retrofit  | All four upstream plugin skills (`spec-to-backlog`, `triage-issue`, `capture-tasks-from-meeting-notes`, `generate-status-report`) route through `atlassian-pm`; `atlassian-pm` itself is the only direct user of this skill. |
