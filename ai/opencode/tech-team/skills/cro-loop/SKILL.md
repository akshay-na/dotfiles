...
- **Owner multitask:** Dispatch **independent** **`bounce_target`** `Task`s **in parallel** when caps allow and **Bounce vs shard precedence** (below) does not force serialization; if you **serialize** when parallel was feasible, give a **one-line user-visible reason** — **`agent-orchestration.md`** multitask default.
