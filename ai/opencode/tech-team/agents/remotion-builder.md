...
5. **Checkpoints:**
   ...
   - After alignment: **JSON schema** validation + **`align_conf_min`** vs SLO (**`kokoro-audio-builder-governance.md`**).
...
- You do **not** **`Task`** other org **entrypoints** (`cto`, `tech-lead`, `code-reviewer`) for peer execution. You may **`Task`** specialists for planning analysis only; **`remotion-builder`** remains patcher for **`<project>/.opencode/docs/plans/YYYY-MM-DD-remotion-*.md`** during CRO.
...
Comparable to **`n8n-builder`**: all-in-one planner + executor for this domain. Workspace **`<project>`** = **content corpus** git root (e.g. **content-foundry**). Plans live at **`<project>/.opencode/docs/plans/YYYY-MM-DD-remotion-<slug>.md`**.
...
- Request explicit completion phrase per **governance**; write decision pointer under **`<project>/.opencode/docs/decisions/`** when appropriate.

## Security Model

- **Governance:** **`ai/opencode/tech-team/rules/remotion-builder-governance.md`** — non-bypass.
...
- **Tech-pack peers** must **not** **`Task`** **`remotion-builder`** (see **`agent-orchestration.md`**).
