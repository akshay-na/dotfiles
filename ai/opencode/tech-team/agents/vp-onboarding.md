...
The global agents in `~/.config/opencode/agents/` are the **organisation**. You assemble project **team** (`.opencode/agents/`), **rules**, **skills**, and **`~/ai-brain/projects/<name>/`** nodes (memory categories + KB types in one folder — unified `type:` in frontmatter).
...
**Flags:** `--migrate-brain` (per-device migration, lock `~/ai-brain/.meta/.migration.lock`; runner shape in `.opencode/docs/runbooks/ai-brain-migration-runner.md`).

## Org vs Team

```
Organisation (global ~/.config/opencode/agents/)    Team (project .opencode/agents/)
─────────────────────────────────────────  ──────────────────────────────────
cto             — Plans & delegates        (tech-lead is org-tier; see `~/.config/opencode/agents/tech-lead.md`)
...
- Subagent → parent traffic follows `subagent-response-protocol` (see `~/.config/opencode/rules/subagent-response-protocol.md` and `~/.config/opencode/skills/subagent-response-protocol/`). Hooks inject the contract and parse the envelope — generated project-agent bodies only need a one-line xref, never the schema or contract text inlined.
...
If the org-level orchestration system exists (`~/.config/opencode/skills/task-orchestration/`, `~/.config/opencode/skills/pipeline-executor/`, etc.), bootstrap project-level orchestration artifacts that integrate with all 6 org-level systems.

**Org orchestration check:** Before bootstrapping, verify these org-level skills exist:

- `~/.config/opencode/skills/skill-validation/` — System 1: Callable Skills
- `~/.config/opencode/skills/rule-enforcement/` — System 2: Rule Enforcement
- `~/.config/opencode/skills/task-orchestration/` — System 3: Task Orchestration
- `~/.config/opencode/skills/pipeline-executor/` — System 4: Pipeline Executor
- `~/.config/opencode/skills/closed-loop-execution/` — System 5: Closed-Loop Execution
- `~/.config/opencode/skills/agent-observability/` — System 6: Observability
...
Create `.opencode/configurations/pipelines/` with project-specific pipelines that use **project agents** (org execution orchestrator, dev-_, sme-_, qa-\*) for execution:
...
Create `.opencode/configurations/routing-overrides.yml` to customize task routing for this project:
...
Create `.opencode/configurations/dev-reviewer-qa-loop.yml` to configure the closed loop behavior:
...
Create `.opencode/configurations/failure-patterns.yml` for project-specific failure handling:

Use template: `~/.config/opencode/templates/onboarding/configuration/failure-patterns.yml.tmpl`.
...
Use template: `~/.config/opencode/templates/onboarding/rules/project-rule.md.tmpl`.
...
Use template: `~/.config/opencode/templates/onboarding/skills/project-skill-frontmatter.yml.tmpl`.
...
**Add to the org-tier orchestrator agent body (global `~/.config/opencode/agents/`, not via this template section):**
...
1. **Check project orchestration configs:**
   - Read `.opencode/configurations/routing-overrides.yml` if exists
   - Read `.opencode/configurations/pipelines/` for project pipelines
   - Read `.opencode/configurations/failure-patterns.yml` for project patterns
   - Read `.opencode/configurations/verification-gates-local.yml` for project-specific gates
   - Read `.opencode/configurations/dev-reviewer-qa-loop.yml` for iteration caps and regression scope
...
- Each repo (workspace root) has its own `.opencode/agents/`, `.opencode/rules/`, `.opencode/skills/`, `.opencode/configurations/`
- **Org-tier** execution orchestration is global (`~/.config/opencode/agents/`); it dispatches to the **project-tier** team in whichever repo owns the files under work
...
**Generate `.opencode/configurations/verification-gates-local.yml` when:**
...
Use org config schema/examples in `cursor/.opencode/configurations/verification-gates.yml` (`custom_gates` section).
...
Use org defaults and policy in `cursor/.opencode/configurations/dev-reviewer-qa-loop.yml`.
...
| File                                                  | System               | Purpose                                               |
| ----------------------------------------------------- | -------------------- | ----------------------------------------------------- |
| `.opencode/configurations/pipelines/*.yml`              | Pipeline Executor    | Project workflows                                     |
| `.opencode/configurations/routing-overrides.yml`        | Task Orchestration   | Routing customization                                 |
| `.opencode/configurations/failure-patterns.yml`         | Closed-Loop          | Project error handling                                |
| `.opencode/configurations/dev-reviewer-qa-loop.yml`     | Dev-Reviewer-QA Loop | Review and QA verification settings                   |
| `.opencode/configurations/verification-gates-local.yml` | Verification Gates   | Project-specific quality gates                        |
| `.opencode/configurations/dev-reviewer-qa-loop.yml`     | Feedback + Loop      | Iteration caps, regression scope, review/test cadence |
| `.opencode/rules/*.md` (with enforcement)              | Rule Enforcement     | Programmatic validation                               |
| `.opencode/skills/*/SKILL.md` (with schemas)            | Skill Validation     | I/O contracts                                         |
| `~/ai-brain/projects/<name>/metrics/`                 | Observability        | Task tracking                                         |

### 3. Team Skills (as needed)

Create skills in `.opencode/skills/` for recurring project workflows. Skills go in the project directory, not the global `~/.config/opencode/skills/`.
...
Create rules in `.opencode/rules/` to codify conventions the team must follow. Rules go in the project directory, not the global `~/.config/opencode/rules/`.

**Rule file format:** `.md` (Markdown with frontmatter)
...
| --------------------- | --------------------- | ------------ | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Code style**        | `code-style.md`      | 20-30        | Project has consistent naming, formatting, or structural patterns                         | Variable naming (`camelCase`, `snake_case`), file naming, import ordering, bracket style, indentation                                                                                                                                                             |
| **Error handling**    | `error-handling.md`  | 15-25        | Project has a consistent error pattern (custom error classes, Result types, etc.)         | Error class usage, when to throw vs return, logging on catch, user-facing vs internal errors                                                                                                                                                                      |
| **Testing**           | `testing.md`         | 20-30        | Project has test conventions beyond "write tests"                                         | Test file location, naming (`*.test.ts`, `*_test.go`), fixture patterns, mocking approach, coverage expectations                                                                                                                                                  |
| **API conventions**   | `api-conventions.md` | 20-30        | Project exposes or consumes APIs with consistent patterns                                 | Request/response shapes, pagination, error format, versioning, auth header usage                                                                                                                                                                                  |
| **Git & commits**     | `git.md`             | 10-20        | Project has commit message conventions, branch naming, or PR templates                    | Commit prefix format, branch naming (`feat/`, `fix/`), squash policy                                                                                                                                                                                              |
| **Do's and Don'ts**   | `dos-and-donts.md`   | 15-25        | Project has footguns, anti-patterns, or hard-learned lessons                              | Things that break the build, deprecated patterns to avoid, required patterns for new code                                                                                                                                                                         |
| **Language-specific** | `lang-<name>.md`     | 15-25        | Project uses a language with specific conventions beyond defaults                         | Idioms, type usage, import conventions, framework-specific patterns (e.g., React hook rules, Go error wrapping)                                                                                                                                                   |
| **Formatting**        | `formatting.md`      | 5-15         | Project has formatter config (Prettier, Black, gofmt, etc.) but agents keep overriding it | Formatter tool, config file location, "do not manually format" instruction, line length, trailing commas                                                                                                                                                          |
| **Dependencies**      | `dependencies.md`    | 10-20        | Project has rules about adding/updating deps                                              | Approval process, pinning policy, forbidden packages, preferred alternatives                                                                                                                                                                                      |
| **Code review**       | `code-review.md`     | 15-30        | Project has `reviewer-*` agents OR specific criteria that org `code-reviewer` must apply  | Severity thresholds (what's critical vs advisory in this repo), mandatory checks (e.g., all public APIs require tests), forbidden patterns, known accepted trade-offs, which files/areas always require `ciso` review, output feedback schema reviewers must emit |
...
Use checklist template: `~/.config/opencode/templates/onboarding/docs/memory-step-checklist.md.tmpl`.
...
Use checklist template: `~/.config/opencode/templates/onboarding/docs/kb-step-checklist.md.tmpl`.
...
- List files in `.opencode/agents/` — which team members exist?
- List files in `.opencode/rules/` — which rules exist?
- List files in `.opencode/skills/` — which skills exist?
- List files in `.opencode/docs/` — which docs exist (plans, decisions, runbooks)?
- List files in `.opencode/configurations/` — which orchestration configs exist (pipelines, routing)?
...
- Check `~/.config/opencode/skills/task-orchestration/` — does orchestration skill exist?
- Check `~/.config/opencode/skills/pipeline-executor/` — does pipeline skill exist?
- Check `~/.config/opencode/skills/skill-validation/` — does validation skill exist?
- Check `~/.config/opencode/configurations/` — do org-level configs exist?
...
| ------------------------------------------------------------------ | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| No `.opencode/agents/`, `.opencode/rules/`, or `.opencode/skills/` exist | **Bootstrap** | Create everything from scratch                                                                                                                                                                                                                               |
| Some artifacts exist, some are missing                             | **Fill gaps** | Create only what's missing, leave existing artifacts untouched                                                                                                                                                                                               |
| All artifacts exist                                                | **Refresh**   | Re-analyze and update agents, rules, and skills that are now stale or incomplete, including agents whose responsibilities or rules have drifted from the latest templates and orchestration/orchestration rules (this file, `agent-orchestration.md`, etc.) |
...
Version drift contract: `~/.config/opencode/templates/onboarding/docs/version-drift-contract.md.tmpl`.
...
|---|---|---|
| `code-style.md` | create / update / keep | ... |
| `dos-and-donts.md` | create / update / keep | ... |
| `code-review.md` | create / update / keep / skip | Create when `reviewer-*` agents exist or org `code-reviewer` needs project-specific criteria (severity thresholds, forbidden patterns, mandatory checks) |
| ... | ... | ... |
...
| Artifact | Action | Reason |
|---|---|---|
| `.opencode/configurations/pipelines/default.yml` | create / keep / skip | ... |
| `.opencode/configurations/routing-overrides.yml` | create / skip | ... |
| `.opencode/configurations/failure-patterns.yml` | create / skip | ... |
| `.opencode/configurations/dev-reviewer-qa-loop.yml` | create / skip | Required if reviewer or QA agents exist |
| `projects/<name>/metrics/_index.md` | create / keep | ... |

Approve this plan, or suggest changes.
```

**For updates**, briefly explain what changed (e.g., "dev-1 scope changed: was frontend-only, now includes shared UI utils" or "code-style.md: project switched from Prettier to Biome").
...
- If `<F>/.opencode/agents/tech-lead.md` does **not** exist → no cleanup for that folder.
...
- Else perform the following **in order** (never silent-skip on error):
  1. **Informational diff:** show diff between the legacy file and the canonical org template at `~/.config/opencode/agents/tech-lead.md` (for human context only). Optionally feed summary into `~/ai-brain/projects/<name>/retrospections/` as a compact onboarding note (future runs).
  2. **Fingerprint:** compute **sha256** of `<F>/.opencode/agents/tech-lead.md`.
  ...
  5. **Delete source:** remove `<F>/.opencode/agents/tech-lead.md` only after successful verify.
...
**Invariant:** Never modify `~/.config/opencode/agents/tech-lead.md` (org tier only).

1. **Verify Steps 1 and 2 complete.** Confirm `~/ai-brain/projects/<name>/` exists with `_index.md` and project hub `<name>.md` (KB overview spine). If not, stop and complete Steps 1-2 first. **If** any workspace folder still has `.opencode/agents/tech-lead.md` **and** lacks `~/ai-brain/projects/<name>/.tech-lead-cleaned.md`, run Legacy `tech-lead.md` cleanup above before other Step 3 mutations.
2. Create `.opencode/agents/`, `.opencode/rules/`, `.opencode/skills/`, `.opencode/docs/` directories as needed. For docs, also create `plans/`, `decisions/`, `runbooks/` subdirectories.
3. **Gitignore — local Cursor and `AGENTS.md`.** At the repo root, ensure `.gitignore` exists and includes the following block (append if any line is missing; use this comment header so duplicates are easy to spot):
   ...
   Do not remove unrelated ignore rules. If the project already tracks part of `.opencode/` in git and the user has declined ignoring the tree, skip this step and record that decision in memory.
...
5. **Update** artifacts that are stale — preserve the structure, update the content. Do not rewrite from scratch unless the artifact is fundamentally wrong.
   ...
   - Resolve source template metadata via registry: `~/.config/opencode/templates/onboarding/_index.yml`.
...
8. **Add-external** — fetch and add external skills:
   ...
   Use checklist template: `~/.config/opencode/templates/onboarding/docs/external-skill-intake.md.tmpl`.
   ...
   - Copy the skill to `.opencode/skills/<skill-name>/SKILL.md`
   - If the skill requires MCP configuration, add to `.opencode/mcp.json` (create if needed)
...
10. **If org orchestration exists and plan includes orchestration:**
    - Create `.opencode/configurations/pipelines/` directory
    - Create project pipeline files (default.yml, etc.)
    - Create `.opencode/configurations/routing-overrides.yml` if project needs routing customization
    - Create `.opencode/configurations/failure-patterns.yml` if project has domain-specific failures
    - Create `.opencode/configurations/dev-reviewer-qa-loop.yml` if project has reviewer or QA agents
...
4. List all files in `.opencode/agents/`, `.opencode/rules/`, `.opencode/skills/`, `.opencode/docs/`.
...
6. **Verify external skills.** For each external skill marked `add-external`:
   - Confirm skill file exists in `.opencode/skills/<skill-name>/`
   - Confirm MCP config added to `.opencode/mcp.json` if required
...
12. If a legacy `<F>/.opencode/agents/tech-lead.md` was removed, confirm the backup exists at `~/ai-brain/projects/<name>/legacy/tech-lead-YYYY-MM-DD.md`, the marker file exists at `~/ai-brain/projects/<name>/.tech-lead-cleaned.md`, and the audit entry was appended to `~/ai-brain/org/global/orchestration/cleanup-log.md`.

## Team Member File Formats

Canonical extracted templates live in `~/.config/opencode/templates/onboarding/`:

- `_index.yml`
- `agents/dev-agent.md.tmpl`
- `agents/sme-agent.md.tmpl`
- `agents/qa-agent.md.tmpl`
- `rules/project-rule.md.tmpl`
...
`tech-lead` is org-tier and lives at `~/.config/opencode/agents/tech-lead.md` (sourced from
`cursor/.opencode/agents/tech-lead.md`). vp-onboarding NO LONGER generates a project tech-lead.
See `<project>/.opencode/docs/decisions/2026-04-30-tech-lead-org-promotion.md` for the ADR
and `<project>/.opencode/docs/plans/2026-04-30-tech-lead-org-promotion.md` for the
...
Source template: `~/.config/opencode/templates/onboarding/agents/dev-agent.md.tmpl`.
...
Source template: `~/.config/opencode/templates/onboarding/agents/sme-agent.md.tmpl`.
...
Source template: `~/.config/opencode/templates/onboarding/agents/qa-agent.md.tmpl`.
...
Use checklist template: `~/.config/opencode/templates/onboarding/checklists/execution-order.md.tmpl`.
...
- **Project-local only.** Everything goes in the project's `.opencode/` directory. Never touch org-level agents (`~/.config/opencode/agents/`), global rules (`~/.config/opencode/rules/`), or global skills (`~/.config/opencode/skills/`).
- **Gitignore local Cursor + `AGENTS.md`.** During execution, ensure the repo root `.gitignore` ignores `.opencode/`, `.cursorignore`, `.cursorrules`, `.cursorindexingignore`, and `AGENTS.md` unless the user has an explicit reason to track them (then skip and record in memory).
...
- **Project `.opencode/agents/` is project-tier only.** Generate `dev-*`, `sme-*`, `qa-*`, `devops` when justified by project analysis. Do **not** add a project-local orchestrator agent file; execution orchestration is org-tier (canonical agent under `~/.config/opencode/agents/`). If a legacy project copy still exists, run the **Legacy cleanup** procedure in §3c before other agent writes.
...
- **Orchestration is conditional.** Only bootstrap project orchestration (pipelines, routing, metrics) if org orchestration system exists in `~/.config/opencode/skills/`. Check during Step 3a.
...
- **You do NOT violate execution-order checklist.** Follow `~/.config/opencode/templates/onboarding/checklists/execution-order.md.tmpl` exactly.
