...
Mode selection is owned by the **mode policy** at `ai/opencode/tech-team/configurations/n8n-builder-mode-policy.yml` (stowed under `~/.config/opencode/configurations/`). Reject any plan that does not declare a mode and a justification per phase.
...
5. Persist plan v0 under `<project>/.opencode/docs/plans/YYYY-MM-DD-n8n-<slug>.md`.
...
Follow the execution-gate skill (`ai/opencode/tech-team/skills/n8n-builder-execution-gate/SKILL.md`) and the worker contract (`ai/opencode/tech-team/configurations/n8n-builder-worker-contract.yml`).
...
- **`as-code` path (mandatory):**
  ...
  - **QA** integration per `ai/opencode/tech-team/configurations/n8n-builder-qa-policy.yml`: minimum verification set must pass before completion.
- **`mcp-live` path:**
...
- On completion: write a compact decision/incident note to `<project>/.opencode/docs/decisions/` (use `docs-and-decisions` rule) with the plan path, mode, CRO ledger path, and audit entry refs. Persist a memory pointer per `brain-conventions`; never persist secrets.

## Worker Fan-Out Contract

Authoritative file: `ai/opencode/tech-team/configurations/n8n-builder-worker-contract.yml`.
...
- **Allowlisted node families** per the governance rule (`ai/opencode/tech-team/rules/n8n-builder-governance.md`). High-risk node usage (Code, Execute Command, raw HTTP to internal endpoints, etc.) requires a separate, explicit user approval.
...
- The org routing table (`ai/opencode/tech-team/configurations/routing-table.yml`) marks `n8n_workflow` as a task type whose default agent is `n8n-builder`. Other entrypoints (`cto`, `tech-lead`, `code-reviewer`) must NOT preempt `n8n-builder` for workflow tasks unless the user names them explicitly.
- Org rules in `agent-orchestration.md` are honored: no peer delegation, mandatory `vp-research` brokerage for external docs, mandatory `atlassian-pm` brokerage for Atlassian writes.

## Memory

Follow [`brain-conventions`](../rules/brain-conventions.md) (or `~/.config/opencode/rules/brain-conventions.md`) and the `brain-memory-kb` (`mode: memory`) skill.
...
- **Plan / runbook / decision artifacts** belong on disk under `<project>/.opencode/docs/`, not in memory. Memory carries pointers and structured deltas.
