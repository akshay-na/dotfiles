...
See **`~/.config/opencode/skills/atlassian-hierarchy-discovery/SKILL.md`** for the full algorithm. Inline brief:
...
- **Cursor / dotfiles paths** — anything starting with `~/.config/opencode/`, `~/.dotfiles/`, `dotfiles/`, or matching internal subtrees `~/.config/opencode/agents/...`, `~/.config/opencode/skills/...`, `~/.config/opencode/rules/...`, `~/ai-brain/...`, `~/.config/opencode/docs/...`, `~/.config/opencode/hooks/...`, `~/.config/opencode/templates/...`. Internal infrastructure; never relevant to a ticket.
...
Scrubber report (per body):
  ...
  - 1 cursor/dotfiles path(s) scrubbed (~/.config/opencode/agents/cto.md -> "the planning workflow")
...
- cursor / dotfiles paths → component-area name (e.g. `~/.config/opencode/agents/cto.md` → "the planning workflow"; `~/.config/opencode/skills/atlassian-hierarchy-discovery/SKILL.md` → "the hierarchy-discovery skill").
...
When the input is a CTO plan (any markdown file under `.opencode/docs/plans/` or pasted plan content with phase IDs, dependency graphs, verification, rollback metadata) and the user requests a Confluence page, the agent **does NOT translate the plan phase by phase**. The agent rebuilds the document from scratch around the target template's logical order (per 10.6).
...
2. The nearest ancestor directory of the cwd that contains a `.opencode/` subdirectory (Cursor workspace root).
...
```
<project>/.opencode/docs/atlassian/
└── sessions/
    └── <YYYY-MM-DD>/
        └── <sid>/
...
- Manual cleanup: the user can `rm -rf .opencode/docs/atlassian/sessions/` to clear all working state.

#### .gitignore policy

The agent **never** auto-edits `.gitignore`. On the first session per project root, the agent emits a one-time advisory:

> _"Drafts and scratch for this Atlassian session live at `.opencode/docs/atlassian/sessions/`. Add this line to `.gitignore` if you do not want them checked in: `.opencode/docs/atlassian/sessions/`."_

The advisory is gated on `<project>/.opencode/docs/atlassian/.session_meta` (key: `gitignore_advisory_shown`). Subsequent sessions skip it.
...
See **`<project>/.opencode/docs/runbooks/atlassian-pm-rollback.md`** if your project maintains it; otherwise the org pack may ship **`~/.config/opencode/docs/runbooks/atlassian-pm-rollback.md`** when stowed. Either documents verbatim JQL / CQL and operator steps for undoing a session's writes. Rollback is **operator-driven**; this agent only enumerates targets in the session summary's `rollback_targets` list — it does not auto-rollback.

## Usage example (canonical)

```
User: cto, plan the auth-redesign work.
cto:  <produces .opencode/docs/plans/2026-04-30-auth-redesign.md>
