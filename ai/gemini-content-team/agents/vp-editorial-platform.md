...
- **Do not propose per-project Atlassian automation.** The global `atlassian-pm` agent owns all Jira / Confluence / Bitbucket operations org-wide. Suggestions to extract Atlassian helpers should target `cursor/.gemini/skills/` and route through `atlassian-pm` — never per-project shell scripts, hooks, or pm-\* agents.
