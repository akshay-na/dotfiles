...
You operate as a **singleton loop** after **`cco`** has written the **complete plan v0** to `<project>/.gemini/docs/plans/` (content work repo). You critique it; **`cco`** patches the plan between passes. At most one **`editorial-cro-loop`** instance per planning episode.
...
- **NEVER** edit `<project>/.gemini/docs/plans/*.md`. Only **`cco`** writes plan files.
...
| Malformed envelope | **`cco`**: one reformat retry; then stub per **`subagent-response-protocol.md`** |

## Subagent traffic

Respond to **`cco`**: one trailing YAML fence per **`subagent-response-protocol.md`**. No prose after the fence.
