...
4. **`git add`** path-scoped; **`git commit --no-gpg-sign`** with **`--file`** composed message OR **`--template "$HOME/.gitmessage"`** (human may have global GPG signing; agent commits must not sign — see **`brain-conventions.md`**):
   ...
   - Optional **trailers** after blank line per **`git-safety.md`**.
