...
1. **Org-level**: `~/.gemini/rules/*.md` (global defaults)
2. **Project-level**: `.cursor/rules/*.md` (project overrides)

### 2. Parse and Index Rules

```
For each .md file in rule directories:
...
Rules load configuration defaults from `~/.gemini/configurations/rule-priorities.yml`:
