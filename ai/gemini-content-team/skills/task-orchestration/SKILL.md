...
output_schema:
  ...
  optional:
    ...
    - name: tech_lead_bootstrapped
      type: boolean
      description: Legacy schema field name; in content org means operator added project agents under .cursor/agents/ when missing
pre_checks:
...
2. **Team availability check** (lazy — if `.cursor/agents/` empty and task mutates repo, surface to operator per Availability Protocol)
...
1. Each repo may have project-level agents under:
   - {repo}/.cursor/agents/editor-*.md
   - {repo}/.cursor/agents/sme-*.md
   - {repo}/.cursor/agents/dev-*.md (optional)

2. Org-level agents live in the **content pack** (stowed ~/.gemini/agents/):
   - cco, content-lead, vp-editorial-*, vp-audience-engineering, vp-editorial-platform,
     editorial-ops-lead, staff-editor, cpo, vp-research, editorial-cro

3. When task spans multiple repos:
   - Create separate task entries for each repo
   - Each subtask routes via **content-lead** using that repo's `.cursor/agents/`
...
# content-lead resolves the project team per target_repo at dispatch time;
# subtasks no longer pin a per-repo content-lead path.
...
Lazy / on-demand: do **not** assume an org onboarding agent — **content org has no `vp-onboarding`**. Evaluate project **team presence** (`sme-*`, `editor-*`, `dev-*` if present, `devops`) under `{repo_root_path}/.cursor/agents/`. If the repo needs typed agents and has none, **ask the user once** to add them or document a manual bootstrap; do not block automation on a missing bootstrap agent.
...
1. Enumerate agents directory:
   ...
   - Matches: editor-*.md, sme-*.md, dev-*.md (if used), devops*.md
...
4. If none exist AND requires_execution_in_repo is true → **surface to user / n8n**: missing project agents; recommend adding `*.md` under `.cursor/agents/` per `content-team-discovery`; optional fallback to org specialists only if the plan allows; log `decision_type:fallback`

5. Return status (never treat org-tier content-lead file under ~/.gemini/agents as repo team presence)
```

### Onboarding Invocation

```yaml
# When project team is missing AND execution is required — content org: no vp-onboarding
manual_project_agents:
  action: user_or_operator_adds_agents_under_repo_cursor
  target_repo: {repo_root_path}
  wait_for_completion: false
  on_success:
    - Re-read {repo}/.cursor/agents/ to discover new agents (editor/sme/dev patterns)
  ...
  on_failure:
    ...
    - Report: "Cannot execute task — no project agents under .cursor/agents/"
    - Suggest: "Add agent markdown files under {repo}/.cursor/agents/ (see content-team-discovery)"
```
...
1. Execution orchestration:
   - Org-tier content-lead (single entrypoint) resolves project team via {repo}/.cursor/agents/ at dispatch time
   - Repo subtasks pin team_path (<repo>/.cursor/agents/), not content-lead file paths

2. Project team: only when `.cursor/agents/` is empty and execution needs locals — surface to operator (content org has no auto-bootstrap agent).

3. Org-level specialists (via **CCO** / **content-lead** delegation):
   - Editorial architecture / IA: vp-editorial-architecture
   - Privacy / claims / sensitive copy: cpo
   - SEO / distribution framing: vp-audience-engineering
   - Research / docs broker: vp-research
   - (No Atlassian; no org qa-* tier.)
   - These advise; they do not replace content-lead execution orchestration inside the repo
```
...
state: running
agents_used: [cco, staff-editor, cpo]
...
|---|---|---|---|
| vp-editorial-architecture | ... | high | ... |
| cpo | ... | critical | ... |
