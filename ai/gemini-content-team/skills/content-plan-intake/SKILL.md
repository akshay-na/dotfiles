...
1. **Project identity** — [`kb-identity`](../kb-identity/SKILL.md) / workspace root / `project_root` from payload. Treat this root as **`<project>`** for **plan output**: **`cco`** must write plans under `<project>/.gemini/docs/plans/` only. If the workspace also includes **`~/.cursor`** opened as a project folder, or **org-pack source** checkouts for editing the pack itself, **do not** choose those as `<project>` for content plans unless the brief is explicitly about editing that pack.
