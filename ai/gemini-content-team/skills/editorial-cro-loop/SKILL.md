...
description: Two-pass editorial plan-critic loop (`editorial-cro`); ledger schema, adversarial rubric, bounce rubric, caps, observability. Content org only — no Atlassian.
version: 1
---

# Editorial CRO loop (`editorial-cro`)

Singleton **execution-boundary** critic: after the **planning-episode owner** (typically **`cco`**) has **plan v0** — **persisted** **or** **prompt-only** — **post–v0 edit round**, and user signals **execution intent**, invoke **once**. Passes: `editorial-cro` pass 1 → **owner** patches v1 → pass 2 → **owner** patches v2 — **before** **`content-lead`**. See [`editorial-cro`](../../agents/editorial-cro.md), [`agent-orchestration`](../../rules/agent-orchestration.md).
...
- **Domain gap** → `bounce_target` set; **owner** dispatches specialist; **no** lateral dispatch from `editorial-cro`.
