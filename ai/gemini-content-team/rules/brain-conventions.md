...
The human may have **`commit.gpgsign=true`** globally. **All commits created by agents or automation in this pack** must **not** invoke GPG. Use **`git commit --no-gpg-sign`** or **`git -c commit.gpgsign=false commit`**. Applies to the **content corpus** repo (`content-git-workflow`), **`~/ai-brain`** when synced below, and any other agent-driven commit.

## Commit message format (`~/.gitmessage` + Conventional Commits)

Use **`$HOME/.gitmessage`** (DotMate: `dotfiles/git/.gitmessage` when stowed). Prefer **`git commit --no-gpg-sign -t "$HOME/.gitmessage"`** or **`--file`** per [**`content-git-workflow`**](../skills/content-git-workflow/SKILL.md).
...
4. **`Notes:`** — include **AI block** when **`content-git-workflow`** / automation requires (`AI-authored`, `Generated-by`, `Task-ID`, `Workflow`); keep other notes **brief**.
