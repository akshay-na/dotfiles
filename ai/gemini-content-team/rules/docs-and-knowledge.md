...
- **Editorial durable paths:** **`<content-brain>/`** = **`~/ai-brain/projects/<project_name>/`** (`rules/project-identity.md`, `rules/brain-paths.md`). **`project_name`** matches Cursor **`kb-identity`** for the **same `project_root`**, so Cursor and Gemini **share** that project subtree under **`~/ai-brain/projects/`** by default.
