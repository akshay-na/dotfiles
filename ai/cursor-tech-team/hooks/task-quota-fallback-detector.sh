#!/usr/bin/env bash
# task-quota-fallback-detector.sh
#
# postToolUse hook (matcher: Task).
# Detects Task failures (quota/model unavailable/interrupt), classifies cause,
...
if [ "$cause" = "no_failure" ]; then
  log_event "allow" "task_no_failure_signal"
  printf '{"permission":"allow"}
'
  exit 0
fi
...
if [ "$cause" = "user_interrupt" ]; then
  jq -n --arg c "$cause" --arg m "task-quota-fallback-detector: classified=$cause. Last Task was user-stopped. No auto-retry attempted."     '{permission:"allow",agent_message:$m,hook_classification:$c}'
  exit 0
fi

jq -n --arg c "$cause" --arg m "task-quota-fallback-detector: classified=$cause. Retry immediately with same Task payload using model:auto. Hook can signal retry, but cannot directly re-run tool call."   '{permission:"allow",agent_message:$m,hook_classification:$c,retry_recommended:true}'
exit 0
