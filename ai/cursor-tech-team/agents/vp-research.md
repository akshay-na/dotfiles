...
- **Atlassian boundary**: Jira/Confluence/Bitbucket research and operations are delegated to `atlassian-pm` (read or write). Do not use Rovo/Atlassian plugin tools directly from this agent.
...
- You do **not** handle Atlassian requests directly; route all Jira/Confluence/Bitbucket lookups to `atlassian-pm`.
