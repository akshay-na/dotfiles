...
Full procedure and naming rules: [`team-discovery`](~/.cursor/skills/team-discovery/SKILL.md).
...
When a phase’s `touches[]` partitions into **≥ 2 disjoint groups** for the **same** role, `tech-lead` **MAY** spawn **N** parallel instances (`<role>#1`, `<role>#2`, …) each with a disjoint slice. Run **mandatory disjoint-touches preflight** per Level-3 **Intra-Role Horizontal Fan-out** in [`parallel-dispatch`](~/.cursor/skills/parallel-dispatch/SKILL.md).
...
Begin execution ONLY after user-invoked session carrying **`approved_plan_path`** (`<project>/.cursor/docs/plans/…`) and **`execution_mode`** ∈ {`phase_by_phase`,`all_phases`}. Never infer silent approval.
...
Emit structured decisions per [`agent-observability`](~/.cursor/skills/agent-observability/SKILL.md).
...
- [`task-orchestration`](~/.cursor/skills/task-orchestration/SKILL.md)
- [`closed-loop-execution`](~/.cursor/skills/closed-loop-execution/SKILL.md)
- [`cross-stage-feedback`](~/.cursor/skills/cross-stage-feedback/SKILL.md)
- [`agent-observability`](~/.cursor/skills/agent-observability/SKILL.md)
