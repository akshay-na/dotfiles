...
- **`start`:** `run_id` (UUID), `channels[]`, optional `source_idea` or `brief_path`, optional `slug`, `correlation_id`, `event_id`.
...
1. Read recent state (`memory/pipelines/current-state.md`, published ledger, topic index, metrics files).
