...
This file is stowed to `~/.gemini/GEMINI.md`. It describes the **content-knowledge-base** subagent setup: one external orchestrator (`cco`), one external metrics agent (`metrics-steward`), and **internal** personas that only run inside a single `cco` invocation.

Canonical paths, symlinks, scripts, and naming: **`CONVENTIONS.md`**.
...
| `metrics-steward` | `agents/metrics-steward.md` | Ingest/normalise manual metrics into `kb/90-Analytics/` and `memory/org/metrics-latest.md`. **Never** run inside `cco`.                                                                                                            |
...
| `editor-blog`               | DRAFT                             | `kb/40-Drafts/blog/`                              |
| `editor-twitter`            | DRAFT                             | `kb/40-Drafts/twitter/`                           |
| `editor-linkedin`           | DRAFT                             | `kb/40-Drafts/linkedin/`                          |
| `editor-shorts`             | DRAFT                             | `kb/40-Drafts/shorts/` (Instagram/Reels map here) |
| `editor-newsletter`         | DRAFT                             | `kb/40-Drafts/newsletter/`                        |
| `qa-content`                | QA_SERIAL                         | Dedup + QA                                        |
| `vp-brand`, `ciso-content`  | BRAND_COMPLIANCE (parallel reads) | Brand + legal/safety                              |
| `archivist`, `kb-librarian` | PUBLISH                           | Ledger, `kb/50-Published/`, metadata              |
| `repurposer`                | REPURPOSE                         | `kb/60-Repurposed/`                               |

Authoritative table: `agents/internal/_index.md`.
...
9. Internal persona bodies from `agents/internal/*.md` when their phase runs.

## Repo hygiene (binding for `cco` and `metrics-steward`)

- **Start:** `git -C "${CONTENT_KB_REPO:-$HOME/content-knowledge-base}" pull --rebase --autostash`
- **End (after writes / webhooks):** `bash "$HOME/.gemini/scripts/kb-sync.sh" sync`

If the runtime cannot execute shell, the wrapper must run these around the model call. Details: `rules/repo-hygiene.md`.

## METRICS_READ

After start-of-run pull, before **BRIEF**: read `kb/90-Analytics/metrics-current.json` and `memory/org/metrics-latest.md`; append **`metrics_snapshot_read`** to `memory/observability/events.jsonl` (and optional per-run log). Repeat before **DRAFT** on `continue` / `revise` or long wall-clock gaps per orchestration. Only **`metrics-steward`** writes `kb/90-Analytics/metrics-*`.
...
## KB and observability

- Path rules: `rules/kb-conventions-content.md`.
...
- Append-only audit: `memory/observability/events.jsonl`; schema `memory/_schema/observability-event.schema.md`.
- Pipeline state / failures: `memory/pipelines/` (FSM, runs ledger, `failures/`).

## Content writes

Prefer normal editor tools for KB files. Do not use shell heredocs to author structured content unless a rule explicitly allows. **`metrics-steward`** alone writes analytics artefacts under `kb/90-Analytics/metrics-*`.

## Gemini CLI coexistence

Gemini CLI may also own `~/.gemini/settings.json`, `extensions/`, OAuth material. This tree uses **`agents/`**, **`rules/`**, **`skills/`** (except `pipeline-state` symlinked from the content repo per `CONVENTIONS.md`), **`templates/`**, **`scripts/`** — avoid colliding names with unrelated extensions.
...
- On fatal errors: `memory/pipelines/failures/<run_id>-<ts>.md`, FSM `failed`, `cco.run.failed`; still run **`kb-sync.sh sync`** when repo files changed so logs and state reach git remote when push works.
